...
export const createLearningSession = async (req, res) => {
  try {
    const { title, description, skillName, sessionType, maxParticipants, scheduledDate, isPrivate, swapRequestId } = req.body;
    const userId = req.user.id;

    // If it's a private session, validate the swap request
    if (isPrivate && swapRequestId) {
      ...
      if (swapRequest.fromUserId !== userId && swapRequest.toUserId !== userId) {
        return res.status(403).json({ message: 'You can only create private sessions for your swap requests' });
      }
    }
    ...
    const session = await prisma.learningSession.create({
      data: {
        ...
        swapRequestId: isPrivate ? swapRequestId : null,
        hostId: userId,
        status: 'SCHEDULED'
      },
      include: {
...
export const getAllLearningSessions = async (req, res) => {
  try {
    const { skill, type, status, privateOnly } = req.query;
    const userId = req.user.id;
    ...
    // Filter private sessions - only show user's own private sessions
    if (privateOnly === 'true') {
      whereClause.OR = [
        { isPrivate: false },
        {
      ...
      // For public sessions, exclude private sessions that don't belong to the user
      whereClause.OR = [
        { isPrivate: false },
        {
          AND: [
